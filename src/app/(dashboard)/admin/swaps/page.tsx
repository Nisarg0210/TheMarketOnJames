...
export default async function AdminSwapsPage() {
    ...
    return (
        <>
            <PageTitle title="Admin: Swap Requests" />
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-gray-50/50 border-b border-gray-100">
                                    <th className="px-4 md:px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Requested By</th>
                                    <th className="px-4 md:px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Shift Date</th>
                                    <th className="px-4 md:px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Time</th>
                                    <th className="px-4 md:px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Reason</th>
                                ...
                                {requests.length === 0 ? (
                                    ...
                                    requests.map((req) => (
                                        <tr key={req.id} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td className="px-4 md:px-6 py-4 font-medium text-blue-700">
                                                {req.shift.startTime} - {req.shift.endTime}
                                            </td>
                                            <td className="px-4 md:px-6 py-4 text-gray-500 italic max-w-xs truncate">
                                                "{req.message}"
                                            </td>
